...
export function login(email, password, expectedRole) {
  return apiFetch('/web-auth/login', { method: 'POST', body: { email, password, expectedRole }, auth: false });
}
...
export function firstLogin(email, temporaryPassword, expectedRole) {
  return apiFetch('/web-auth/first-login', {
    method: 'POST',
    body: { email, temporaryPassword, expectedRole },
    auth: false,
  });
}
