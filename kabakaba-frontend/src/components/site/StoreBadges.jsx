 ...
 * le pied de page et la modale ambassadeur. Icônes en SVG inline (poids
 * négligeable, pas de requête réseau supplémentaire).
 *
 * L'app n'étant pas encore publiée sur les stores, les badges ouvrent un
 * popup "bientôt disponible" au lieu de pointer vers un store réel.
 */
...
export default function StoreBadges({ className = '' }) {
  ...
  return (
    <div className={`store-badges ${className}`}>
      <button type="button" className="store-badge" onClick={() => setShowComingSoon(true)}>
